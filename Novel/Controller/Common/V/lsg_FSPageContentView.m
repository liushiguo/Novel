...
@interface lsg_FSPageContentView ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation lsg_FSPageContentView
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(lsg_FSContentViewWillBeginDragging:)]) {
        [self.delegate lsg_FSContentViewWillBeginDragging:self];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(lsg_FSContentViewDidScroll:startIndex:endIndex:progress:)]) {
        [self.delegate lsg_FSContentViewDidScroll:self startIndex:startIndex endIndex:endIndex progress:progress];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(lsg_FSContenViewDidEndDecelerating:startIndex:endIndex:)]) {
        [self.delegate lsg_FSContenViewDidEndDecelerating:self startIndex:startIndex endIndex:endIndex];
    }
}
...
{
    if (!decelerate) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(lsg_FSContenViewDidEndDragging:)]) {
            [self.delegate lsg_FSContenViewDidEndDragging:self];
        }
    }
}
